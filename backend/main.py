...
app = FastAPI(title="RAG Ollama API", version="1.0.0")  # Create a FastAPI Server Instance
...
async def health_check():
    return {"status": "healthy", "message": "RAG Ollama API is running"}
...
async def upload_files(files: List[UploadFile] = File(...)):
    global vectorstore, retriever, workflow_engine
    ...
    # Initialize the workflow after vectorstore is created
    workflow_engine = create_workflow()

    return {"status": "success", "message": "Files uploaded and processed successfully"}


def create_workflow():
    """Create and return the RAG workflow"""
    global retriever
    if not retriever:
        raise Exception("Retriever not initialized. Please upload files first.")
...
# This endpoint processes the question using the RAG workflow
@app.post("/process/")
async def process_question():
    global workflow_engine
    try:
        if not question:
            raise HTTPException(status_code=400, detail="No question provided. Please submit a question first.")
        
        if not workflow_engine:
            raise HTTPException(status_code=400, detail="Workflow not initialized. Please upload files first.")
        
        result = None
        for output in workflow_engine.stream({"question": question}):
        ...
        if result:
            ...
            return {"status": "error", "message": "No generation produced"}
    except Exception as e:
